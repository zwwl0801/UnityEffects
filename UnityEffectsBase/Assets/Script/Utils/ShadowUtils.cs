...
namespace Assets.Script.Utils
{
    public static class ShadowUtils
    {
        ...
        public static void SetLightCamera(Camera mainCamera, Camera lightCamera)
        {
            ...
            //本来这里的矩阵使用mainCamera.cameraToWorldMatrix,
            //但是请看：http://docs.unity3d.com/ScriptReference/Camera-cameraToWorldMatrix.html   
            //cameraToWorldMatrix返回的是GL风格的camera空间的矩阵，z是负的，跟untiy编辑器中的不对应，
            //（也是坑爹的很，就不能统一吗），所以我们直接使用localToWorldMatrix
        ...
        public static void SetLightCamera(Bounds b, Camera lightCamera)
        {
            ...
            //但是同上面不使用mainCamera.cameraToWorldMatrix的原因一样，
            //我们直接使用worldToLocalMatrix
            ...
            //4.5 优化，
            //如果8个顶点在光锥view空间中的z<0,
            //那么如果n=0，就可能出现应该被渲染depthmap的物体被光锥<近裁面>剪裁掉的情况，
            //所以z < 0 的情况下要延光照负方向移动光源位置以避免这种情况
            if (minZ < 0)
            {
                lightCamera.transform.position += -lightCamera.transform.forward.normalized * Mathf.Abs(minZ);
                maxZ = maxZ - minZ;
