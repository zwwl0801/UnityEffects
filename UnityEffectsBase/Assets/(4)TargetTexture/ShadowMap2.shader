...
Shader "UnityEffects/ShadowMap2"
{
	//渲染接受阴影物体
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float4 shadowCol = depthPixel - depth ;
				//靠近相机的，depthPixel - depth > _bias
				//远离相机的，depthPixel - depth < _bias
				//return textureCol * (shadowCol);
				return fixed4(1,1,1,1)* (shadowCol);
				//这个shader的明暗是反应的深度关系，离摄像机越是近z值越是小，越是会显现出黑色。
